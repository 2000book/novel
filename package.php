...
'.$textwatermark.'<br />'.$v;
	...
	function makeIndex($dynamic=false, $show=false)
	{
		...
		//封面URL
		$_17mb_url = constant("JIEQI_URL");
		$_17mb_shortid = intval($this->id/1000);
		$_17mb_aid = intval($this->id);
		$_17mb_url_img_0 = "../../files/article/image/".$_17mb_shortid."/".$_17mb_aid."/".$this->id."s.jpg";
		$_17mb_url_img = $_17mb_url."/files/article/image/".$_17mb_shortid."/".$_17mb_aid."/".$this->id."s.jpg";
		$_17mb_url_img = file_exists($_17mb_url_img_0) ? $_17mb_url_img : $_17mb_url."/files/article/images/nocover.jpg" ; 
		//获取值
		$_17mb_1 = @mysql_query("select fullflag,lastupdate,size from `jieqi_article_article` where articleid = '".$_17mb_aid."' ");
		$_17mb_2 = @mysql_query("select chapterid,chaptername from `jieqi_article_chapter` where chaptertype = '0' and articleid = '".$_17mb_aid."' order by chapterorder desc limit 0,9");
		$_17mb_fullflag = mysql_result($_17mb_1,'0','fullflag');
		$_17mb_uptime = mysql_result($_17mb_1,'0','lastupdate');
		$_17mb_size = mysql_result($_17mb_1,'0','size');
		if($num = mysql_num_rows($_17mb_2))
		{
			$i = 0;
			while($row = @mysql_fetch_array($_17mb_2))
			{
				$_17mb_9[$i]['chaptername'] .= $row['chaptername'];
				$_17mb_9[$i]['chapterid'] .= $row['chapterid'];
				$i++;
			}
		}
		//整理值
		$_17mb_fullflag = ($_17mb_fullflag == 1) ? '已完结' : '连载中' ;
		$_17mb_uptime1 = date('Y-m-d H:i:s',$_17mb_uptime);
		
		//模板赋值
		$jieqiTpl->assign('_17mb_url_img', $_17mb_url_img);
		$jieqiTpl->assign('_17mb_fullflag',$_17mb_fullflag);
		$jieqiTpl->assign('_17mb_uptime',$_17mb_uptime);
		$jieqiTpl->assign('_17mb_uptime1',$_17mb_uptime1);
		$jieqiTpl->assign('_17mb_size',$_17mb_size);
        $jieqiTpl->assign('_17mb_size_k',ceil($_17mb_size/1024));
        $jieqiTpl->assign('_17mb_size_c',ceil($_17mb_size/2));
		$jieqiTpl->assign('_17mb_9',$_17mb_9);
		// mulu end
